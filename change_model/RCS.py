...
class Bottleneck_RepVGG(nn.Module):
    """Standard bottleneck."""
...
class C3k2_RepVGG(C2f):
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_RepVGG(self.c, self.c, shortcut, g) for _ in
            range(n)
        )


if __name__ == '__main__':
    RepVGG = RepVGG(256, 128)
    # 创建一个输入张量
    batch_size = 8
    input_tensor = torch.randn(batch_size, 256, 64, 64)
    # 运行模型并打印输入和输出的形状
    output_tensor = RepVGG(input_tensor)
